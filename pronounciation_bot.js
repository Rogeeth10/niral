...
let selectedStatement = ''; // Track the selected pronunciation statement
...
function startChat() {
  chatContent.innerHTML = `
    <div class="message bot-message">
      <span>Hello, How can I assist you today with pronunciation?</span>
    </div>`;
}
...
function handleUserInput() {
  ...
  if (userInput) {
    ...
    userMessage.innerHTML = `<span>${userInput}</span>`; // Corrected string interpolation
    ...
    if (userInput === 'no') {
      ...
      setTimeout(() => {
        if (step === 0) {
          botMessage('Great! Let me show you some pronunciation tips. Please wait...');
          displayPronunciationStatements();
          step = 1;
        } else if (step === 1 && (userInput === 'yes' || userInput === 'no')) {
          handleExplanationChoice(userInput);
          step = 2;
        } else if (step === 2 && userInput === 'yes') {
          displayPronunciationStatements(); // Show 5 statements again
...
// Display rotating cube loading animation before showing pronunciation tips
function displayPronunciationStatements() {
  ...
  setTimeout(() => {
    ...
    botMessage('Here are some pronunciation tips and statements:');

    let statements = [
      "Pronunciation is more than just the correct production of sounds; it plays a crucial role in making speech understandable and clear to listeners.",
      "A person's accent and pronunciation can reveal much about their cultural background, but it should never be a barrier to effective communication.",
      "Mastering the rhythm, stress, and intonation patterns of a language is as important as learning its vocabulary and grammar.",
      "While English spelling often doesn't align with its pronunciation, paying attention to phonetic rules can significantly improve clarity and confidence when speaking.",
      "Improving pronunciation is a lifelong process that requires regular practice, active listening, and an openness to feedback."
    ];
...
// Select a pronunciation statement
function selectStatement(statement) {
  ...
  selectedMessage.innerHTML = `<span>${statement}</span>`; // Corrected string interpolation
...
function evaluateRecording() {
  ...
  if (recordingDuration < 5) {
    // Do not show "too short" message, simply evaluate as not correct
    botMessage('Your pronunciation needs improvement. Keep practicing!');
  } else if (recordingDuration >= 6) {
    botMessage('Excellent pronunciation! Keep it up.');
  }
...
function handleExplanationChoice(choice) {
  if (choice === 'yes') {
    displayPronunciationStatements(); // Restart the process by showing the 5 statements again
...
function botMessage(message) {
  ...
  botMessageDiv.innerHTML = `<span>${message}</span>`; // Corrected string interpolation
